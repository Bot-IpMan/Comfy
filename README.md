...
- `CLI_ARGS="--listen --port 8188 --lowvram --disable-smart-memory --disable-cuda-malloc --fp32-text-enc --preview-method none"` — вимикає попередній перегляд (звільняє VRAM), змушує ComfyUI тримати лише поточну модель у памʼяті, забороняє smart-memory, примушує використовувати `cudaMalloc` та залишає FP32 лише для текстового енкодера CLIP.
...
За відсутності доступного GPU скрипт сам додає `--cpu`, щоб ComfyUI коректно працював у CPU-режимі, тоді як CLIP залишається у FP32 завдяки прапору `--fp32-text-enc`.

Починаючи з CUDA 12, NVIDIA перестала постачати `libnvrtc-builtins-sm_61.so`, потрібну для nvRTC на Pascal (SM 6.1). Dockerfile цього репозиторію встановлює пакети `cuda-nvrtc-12-4`/`cuda-nvrtc-dev-12-4` і створює симлінк `libnvrtc-builtins-sm_61.so` до базової `libnvrtc-builtins.so`. Завдяки цьому PyTorch знову може JIT-компілювати ядра під 6.1 і не падає з помилкою `libnvrtc-builtins-sm_61.so: cannot open shared object file`.
